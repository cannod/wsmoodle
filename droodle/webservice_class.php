...
class droodle_webservice {
    ...
    function list_courses ($available = 0, $sortby = 'created', $guest = 0, $username = '')
	{
		...
		foreach ($records as $curso)
		{
			...
			{
				if (($instance->enrol == 'paypal') || ($instance->enrol == 'droodle'))
				{
			...
			$c['summary'] = str_replace ('pluginfile.php', '/auth/droodle/pluginfile_droodle.php', $c['summary']);
    ...
    function get_course_categories ($cat = 0) 
	{
		...
		foreach ($records as $cat)
		{
			...
			$c['description'] = str_replace ('pluginfile.php', '/auth/droodle/pluginfile_droodle.php', $c['description']);
    ...
    function courses_by_category ($category, $available = 0, $username = '')
	{
		...
		foreach ($records as $curso)
		{
			...
			$c['cat_description'] = str_replace ('pluginfile.php', '/auth/droodle/pluginfile_droodle.php', $c['cat_description']);
    ...
    function get_course_info ($id, $username = '') 
	{
		...
		$course_info['cat_description'] = str_replace ('pluginfile.php', '/auth/droodle/pluginfile_droodle.php', $course_info['cat_description']);
		...
		$course_info['summary'] = str_replace ('pluginfile.php', '/auth/droodle/pluginfile_droodle.php', $course_info['summary']);
    ...
    function get_course_contents ($id) {
		...
		foreach ($records as $r)
		{
			...
			$e['summary'] = str_replace ('pluginfile.php', '/auth/droodle/pluginfile_droodle.php', $e['summary']);
...
function migrate_to_droodle ($username)
{
	global $DB;
	$username = utf8_decode ($username);
	$username = strtolower ($username);
	$conditions = array ('username' =>  $username); 
	$DB->set_field('user', 'auth', 'droodle', $conditions);
	...
	function add_parent_role ($child, $parent)
	{
		...
		$parent_role_id = get_config('auth/droodle', 'parent_role_id');
		if (!$parent_role_id)
            return false;
...
function question_rewrite_question_urls($text, $file, $contextid, $component,
    ...
    function quiz_get_answers ($id)
    {
        ...
        foreach ($records as $record)
        {
            ...
            $r['answer'] = str_replace ('pluginfile.php', '/auth/droodle/pluginfile_droodle.php', $r['answer']);
    ...
    function quiz_get_random_question ($cat_id, $used_ids)
    {
        ...
        $r['questiontext'] = str_replace ('pluginfile.php', '/auth/droodle/pluginfile_droodle.php', $r['questiontext']);
   ...
   function get_course_mods ($id, $username = '') {
        ...
        foreach ($sections as $section)
        {
            ...
            foreach ($sectionmods as $modnumber) {
				...
				$e[$section->section]['summary'] = str_replace ('pluginfile.php', '/auth/droodle/pluginfile_droodle.php', $e[$section->section]['summary']);
				$resource['id'] = $mod->id;
				$resource['name'] = $mod->name;
...
*/
   ...
   function add_cohort_member ($username, $cohort_id)
    ...
    function create_course ($course_ext,$skip_fix_course_sortorder=0){
        ...
        if ($newcourseid = $DB->insert_record("course", $course)) {  // Set up new course
            ...
            $section->id = $DB->insert_record("course_sections", $section);
              add_to_log($newcourseid, "course", "new", "view.php?id=$newcourseid", "auth/droodle auto-creation");
        } else {
            error_log("Could not create new course from droodle");
	...
	function logoutpage_hook() {
		...
		$redirect = get_config (NULL, 'joomla_url').'/components/com_droodle/views/wrapper/getout.php';
	}
	...
	function user_authenticated_hook (&$user, $username, $password)
	{
		...
		$redirectless_sso = get_config('auth/droodle', 'redirectless_sso');
		...
		$redirect_url = get_config (NULL, 'joomla_url').'/index.php?option=com_droodle&view=droodle&task=login&data='.$login_data.'&wantsurl='. urlencode ($SESSION->wantsurl);
    ...
    function log_into_joomla ($username, $password)
    {
		...
		$login_data = base64_encode ($username.':'.$password);
		$url = get_config (NULL, 'joomla_url').'/index.php?option=com_droodle&view=droodle&task=login&data='.$login_data;
	...
	function update_joomla_sessions ()
	{
		...
		$query = "SELECT username FROM {$CFG->prefix}user WHERE auth = 'droodle' and lastaccess > '$cutoff';"; ///XXX PREFIX
		$records = $DB->get_records_sql($query);
		$usernames = array();
		foreach ($records as $record)
	...
	function droodle_user_updated ($user)
	{
                global $CFG, $DB;
				...
				$auth_droodle = new auth_plugin_droodle ();
        ...
        //      $userinfo['password2'] = $password_clear;
                ...
                $auth_droodle->call_method ("updateUser", $userinfo);
	...
	function droodle_user_created ($user)
	{
		global $CFG, $DB;
		...
		$auth_droodle->call_method ("createUser", $userinfo);

		return true;
	}

	function droodle_course_created ($course)
	{
		global $DB;

		$activities = get_config('auth/droodle', 'jomsocial_activities');
		$groups = get_config('auth/droodle', 'jomsocial_groups');
		$autosell = get_config('auth/droodle', 'auto_sell');
		$joomla_user_groups = get_config('auth/droodle', 'joomla_user_groups');
		$use_kunena_forums = get_config('auth/droodle', 'use_kunena_forums');
		...
		$course->summary = str_replace ('pluginfile.php', '/auth/droodle/pluginfile_droodle.php', $course->summary);
		if ($activities)
			$auth_droodle->call_method ('addActivityCourse', (int) $course->id, $course->fullname,  $course->summary, (int) $course->category, $cat->name);
		if ($groups)
		 	$auth_droodle->call_method ('addJSGroup', $course->fullname,  get_string('auth_joomla_group_for_course', 'auth_droodle') . ' ' .$course->fullname,  (int) $course->id, "x");

		if ($autosell)
		{
			$cid = array ($course->id);
			$auth_droodle->call_method ("sellCourse", array ((int) $course->id));
		}

		if ($joomla_user_groups)
		{
			$auth_droodle->call_method ('addUserGroups', (int) $course->id, $course->fullname);
		}

		if ($use_kunena_forums)
		{
			// Create section
			$auth_droodle->call_method ('addForum', (int) $course->id, (int) -2, $course->fullname);
			// Create news forum
	//		$auth_droodle->call_method ('addForum', (int) $course->id, (int) -1, get_string('namenews', 'forum'));
		}

		return true;
	}

	function droodle_course_deleted ($course)
	{
		$groups_delete = get_config('auth/droodle', 'jomsocial_groups_delete');
		$autosell = get_config('auth/droodle', 'auto_sell');
		$use_kunena_forums = get_config('auth/droodle', 'use_kunena_forums');
		$joomla_user_groups = get_config('auth/droodle', 'joomla_user_groups');
		...
		if ($groups_delete)
			$auth_droodle->call_method ('removeJSGroup', $course->fullname);

		if ($autosell)
		{
			$cid = array ($course->id);
			$auth_droodle->call_method ("deleteCourseShop", array ((int) $course->id));
		}

		if ($joomla_user_groups)
			$auth_droodle->call_method ("removeUserGroups", (int) $course->id);

		if ($use_kunena_forums)
			$auth_droodle->call_method ("removeCourseForums", (int) $course->id);

		return true;
	}

	function droodle_course_updated ($course)
	{
		$groups = get_config('auth/droodle', 'jomsocial_groups');
		$autosell = get_config('auth/droodle', 'auto_sell');
		...
		if ($groups)
		 	$auth_droodle->call_method ('updateJSGroup', $course->fullname,  get_string('auth_joomla_group_for_course', 'auth_droodle') . ' ' .$course->fullname,  (int) $course->id, "x");

		if ($autosell)
		{
			$cid = array ($course->id);
			$auth_droodle->call_method ("updateCourseShop", array ((int) $course->id));
		}
	...
	function droodle_role_assigned ($role)
	{
		global $CFG, $DB;

		$activities = get_config('auth/droodle', 'jomsocial_activities');
		$groups = get_config('auth/droodle', 'jomsocial_groups');
		$enrol_parents = get_config('auth/droodle', 'enrol_parents');
		$parent_role_id = get_config('auth/droodle', 'parent_role_id');
		$points = get_config('auth/droodle', 'give_points');
		$auto_mailing_lists = get_config('auth/droodle', 'auto_mailing_lists');
		$use_kunena_forums = get_config('auth/droodle', 'use_kunena_forums');
		$joomla_user_groups = get_config('auth/droodle', 'joomla_user_groups');
		...
		if ($context->contextlevel == CONTEXT_COURSE)
		{
			...
			if ($activities)
			{
				...
				$auth_droodle->call_method ('addActivityCourseEnrolment', $user->username, (int) $courseid, $course->fullname, (int) $course->category, $cat->name);
			}
			...
			if ($groups)
			{
				...
				if ($role->roleid == 3) //XXX not hardcoded value?
					$auth_droodle->call_method ('addJSGroupMember', $course->fullname, $user->username, 1, (int) $courseid);
				else 
					$auth_droodle->call_method ('addJSGroupMember', $course->fullname, $user->username, -1, (int) $courseid);
			}
			...
			if (($enrol_parents) && ($parent_role_id))
			{
				if ($role->roleid == 5) //XXX not hardcoded value?
				{
					...
					foreach ($mentors as $mentor)
					{
						...
						$auth_droodle->enrol_user ($parent_user->username, $courseid, $parent_role_id);
					}
				}
			}

			if ($points)
				$auth_droodle->call_method ('addPoints', 'droodle.enrol', $user->username,   (int) $courseid, $course->fullname);
			...
			if ($auto_mailing_lists)
			{
				...
				else  if ($role->roleid == 5)
				{
					...
					foreach ($mentors as $mentor)
					{
						...
						$auth_droodle->call_method ('addMailingSub',  $parent_user->username,   (int) $courseid, 'course_parents');
					}

				}

				if ($type)
					$auth_droodle->call_method ('addMailingSub',  $user->username,   (int) $courseid, $type);
			}
			...
			if ($joomla_user_groups)
			{
				...
				if ($type)
					$auth_droodle->call_method ('addGroupMember',  (int) $courseid, $user->username, $type);
			}

			if ($use_kunena_forums)
			{
				if ($role->roleid == 3)
					$auth_droodle->call_method ('addForumsModerator',  (int) $courseid, $user->username);
			}
		}
	...
	function droodle_role_unassigned ($role)
	{
		global $DB, $CFG;
		...
		if ($context->contextlevel == CONTEXT_COURSE)
		{
			...
			if ($groups)
				$auth_droodle->call_method ('removeJSGroupMember', $course->fullname, $user->username);
			...
			if ($auto_mailing_lists)
			{
				...
				else  if ($role->roleid == 5)
				{
					...
					foreach ($mentors as $mentor)
					{
						...
						$auth_droodle->call_method ('removeMailingSub',  $parent_user->username,   (int) $courseid, 'course_parents');
					}

				}

				$auth_droodle->call_method ('removeMailingSub',  $user->username,   (int) $courseid, $type);
			}
			...
			if ($joomla_user_groups)
			{
				...
				if ($type)
					$auth_droodle->call_method ('removeGroupMember',  (int) $courseid, $user->username, $type);
			}

			if ($use_kunena_forums)
			{
				if ($role->roleid == 3)
					$auth_droodle->call_method ('removeForumsModerator',  (int) $courseid, $user->username);
			}
		}
	...
	function droodle_quiz_attempt_submitted ($event)
	{
		global $DB , $CFG;
        ...
        if ($activities)
            $auth_droodle->call_method ('addActivityQuizAttempt', $user->username, (int) $event->courseid, $course->fullname,  $quiz->name);

        if ($points)
				$auth_droodle->call_method ('addPoints', 'droodle.quiz_attempt', $user->username,   (int) $event->courseid, $course->fullname);

		return true;
	}

	function droodle_mod_created ($event)
	{
		$use_kunena_forums = get_config('auth/droodle', 'use_kunena_forums');
		...
		if ($use_kunena_forums)
		{
			...
			{
				$auth_droodle->call_method ('addForum', (int) $event->courseid, $event->cmid, $event->name);
			}
		}
	...
	function droodle_mod_deleted ($event)
	{
		$use_kunena_forums = get_config('auth/droodle', 'use_kunena_forums');
		...
		if ($use_kunena_forums)
		{
			if ($event->modulename == 'forum')
			{
				$auth_droodle->call_method ("removeForum", (int) $event->courseid, $event->cmid);
			}
		}
	...
	function droodle_mod_updated ($event)
	{
		$use_kunena_forums = get_config('auth/droodle', 'use_kunena_forums');
		...
		if ($use_kunena_forums)
		{
			if ($event->modulename == 'forum')
			{
				$auth_droodle->call_method ("updateForum", (int) $event->courseid, $event->cmid, $event->name);
			}
		}
	...
	function droodle_course_completed ($event)
	{
		global $DB , $CFG;
        ...
        if ($activities)
            $auth_droodle->call_method ('addActivityCourseCompleted', $user->username, (int) $event->course, $course->fullname);

        if ($points)
				$auth_droodle->call_method ('addPoints', 'droodle.course_completed', $user->username,   (int) $event->courseid, $course->fullname);
